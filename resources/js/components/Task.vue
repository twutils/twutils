...
<template>
  <div class="task task__container">
      ...
      <div class="col col-sm-12">
        <div :class="`border-radius-1rem d-flex flex-column flex-sm-row justify-content-between alert alert-${task.status === 'completed' ? 'success' : (task.status === 'broken' ? 'danger' : 'primary')} text-${isRtl ? 'right' : 'left' }`">
          ...
          <div class="d-flex flex-row flex-sm-column justify-content-between align-items-end" style="min-width: 60px;">
            ...
            <portal-target v-if="!isLocal" name="downloadTask"></portal-target>
          </div>
        </div>
      </div>
    </div>
    ...
    <portal v-if="task != null" to="downloadTask">
      <div :class="`btn-group float-${isRtl? 'left':'right'} ltr`" v-if="!isLocal">
        <a :href="`${window.TwUtils.baseUrl}task/${task.id}/download${isMediaTask(task) ? '/html' : ''}`" :class="`btn btn-outline-primary btn-sm d-flex justify-content-center align-items-center`" download target="_blank">
        ...
        <div class="modal-dialog" role="document">
          <div class="modal-content">
            <div class="modal-header">
              <h4 class="modal-title">{{__('remove')}} ({{__(task.type)}})</h4>
